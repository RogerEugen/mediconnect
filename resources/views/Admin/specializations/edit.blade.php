    ...
    <div class="py-6">
        <div class="max-w-xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6">
                <form action="{{ route('admin.specializations.update', $specialization) }}" method="POST" class="space-y-5">
                    @csrf @method('PUT')
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="3"
                                  class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:ring-purple-500 focus:border-purple-500 text-sm @error('description') border-red-400 @enderror">{{ old('description', $specialization->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
